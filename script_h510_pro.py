...
def _detect_bluetooth_card() -> str | None:
    """Retorna o nome da placa Bluetooth se o headset estiver em modo BT com A2DP disponível."""
    try:
        ...
        logger.error("Timeout ao verificar cartões Bluetooth.")
    except Exception as e:
        logger.error(f"Erro ao detectar Bluetooth: {e}")
    return None


def detect_mode() -> tuple[str | None, str | None]:
    """
    ...
    """
    bt_card = _detect_bluetooth_card()
    if bt_card:
...
def enforce_a2dp_profile(card_name: str) -> None:
